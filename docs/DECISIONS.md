...
**Status:** Accepted (2026-08-20); implemented and tested (Milestone 9, 2026-08-20)
...
**Decision:** Soft-delete via a `status = ARCHIVED` flag (`RetentionService`, `POST /audit/retention/apply`, `AUDITOR`-only, no scheduler since the assignment doesn't require one). Eligibility is based on `recorded_at` (server truth), not the caller-supplied `event_timestamp` — otherwise a caller could game retention by submitting an artificially old timestamp (consistent with `ADR-002`'s reasoning). The row remains in `audit_record`; nothing about its stored content changes, so `/audit/verify` requires no special handling for archived records beyond recognizing the status.

**Addendum (found during implementation):** a record can independently be both "past the retention window" and "redacted" (`ADR-003`) — these are separate concerns tracked by different fields (`status` vs. `redacted_fields`), but `status` is a single-value column. Decision: archival (`AuditRecordEntity.archive()`) only ever flips `ACTIVE` → `ARCHIVED`; it is a no-op if the record is already `REDACTED`, so redaction is never silently overwritten. Consequence: a redacted-and-also-past-window record continues to display `status = REDACTED`, not `ARCHIVED` — archival eligibility for such a record is still determinable from `recorded_at` directly, just not visible from `status` alone. This is a deliberate, documented simplification, not a bug; a system needing both facts simultaneously visible would need `status` to become a set of independent flags rather than one enum, which was judged unnecessary complexity for this prototype's scope.

**Alternatives considered:**
- **Hard archival**, moving rows out of the primary table (or exporting and purging them), with a synthetic "archival manifest" event (e.g. `eventType = SYSTEM_ARCHIVAL`) written into the chain so `/audit/verify` can recognize the resulting gap as legitimate rather than as tampering. More realistic for a real production retention policy, and a stronger demonstration of handling a genuine gap in the chain. Not chosen for this prototype given the time-box: it requires an additional table or export/purge mechanism, a manifest-verification code path, and its own test coverage, for a requirement that soft-delete already satisfies (`FR-B1`'s acceptance criteria only require no false positive on verify, which soft-delete meets directly).
...
**Validated by:** `RetentionTest` (6 tests) — archival of eligible records, no false positive on `/audit/verify`, idempotent re-application, `AUDITOR`-only enforcement, and the redacted-status-preservation addendum above. Also confirmed live via `curl` with `windowDays=-1`.
...
## ADR-013 — Signed export manifests and trust model

**Status:** Accepted (2026-08-20, implemented and tested in Milestone 9)

**Context:** `FR-B3` requires the export bundle (`GET /audit/export`) to be self-contained and independently verifiable. The original design (`docs/ARCHITECTURE.md`, written during planning) proposed an unsigned `SHA256` over the concatenated record hashes — this lets a recipient confirm the bundle's *internal* consistency (nothing was altered after export) but proves nothing about *who produced it*. `docs/EVALUATION_CLOSURE_MATRIX.md` item 15 (`ARC-03`, P2) flags exactly this as an unsigned trust boundary that needs strengthening.

**Decision:** Sign the export manifest asymmetrically (`SHA256withECDSA`, EC P-256 curve) via `ExportSigningService`. Every export response includes the algorithm name, the signer's public key (X.509-encoded, base64), and the signature over a canonical manifest string built purely from the bundle's own published fields (`exportedAt`, `tenantId`, `recordCount`, chain-context fields, and each record's `recordHash` in export order). A recipient needs **only the exported JSON and the published public key** to verify — no access to this server, no shared secret. `ExportBundleService.canonicalManifest()` is `public static` specifically so it documents, by being directly reusable, exactly what a recipient's own reimplementation should reconstruct.

**Trust model, stated explicitly:**
- The signing key pair is generated **fresh on every application startup** and lives only in memory — never persisted, never committed to source control (`docs/SECURITY.md`/`SEC-09` — "do not commit secrets" extends naturally to private keys). This is a deliberate trade-off: a real deployment would source a persistent signing key from a secret manager/HSM. Documented, load-bearing consequence: **a signature cannot be re-verified against the same public key after this application restarts**, since the key pair changes. For a real production system, this would need a persistent key with a documented rotation policy; for this prototype, regenerating on restart is safer than the alternative of embedding a fixed private key in the repository.
- What a valid signature proves: the manifest was produced by whoever held the private key at signing time, and hasn't been altered since. What it does **not** prove: that the exported records are genuinely part of the *full*, live chain (as opposed to a subset independently fabricated by whoever had write access to this service at some point) — verifying inclusion in the full chain would need a periodically published chain root (Merkle-style) or trusting the exporting system, exactly as the original `docs/ARCHITECTURE.md` design already flagged as a scoped limitation, unchanged by adding signing.
- A recipient's own hash/linkage verification within the bundle (walking `previousHash` across the exported records, recomputing each `recordHash`) is unaffected by signing and remains the recipient's independent check that the *content* wasn't altered — signing adds provenance on top, it doesn't replace that check.

**Alternatives considered:**
- **HMAC (symmetric) signing.** Simpler (no key pair, no `Signature` API), but the recipient would need the *same secret* the server used, which either requires a pre-shared secret (awkward for an arbitrary external recipient) or defeats the "prove who produced it" goal entirely (anyone who could verify could also forge). Rejected: asymmetric signing is the correct primitive for "a stronger trust boundary" specifically because verification and signing require different keys.
- **A persistent signing key loaded from configuration.** Would survive restarts, letting old signatures stay verifiable. Rejected for this environment: no secret manager is available, and a persistent key would either need to be committed (unacceptable, `SEC-09`) or provided via an environment variable with no real secret-management story behind it — an ephemeral in-memory key is more honest about what this prototype actually provides.

**Trade-offs / consequences:** the "signature survives restart" limitation above is real and would matter for any long-lived export (e.g., handed to a regulator who verifies it a week later, after the service has restarted). Documented as a scoped, known limitation rather than solved.

**Validated by:** `ExportTest` (6 tests): filter validation, correct record scoping, chain-context correctness, independent signature verification using only published fields (no server access), a tampered-bundle signature-verification failure, and tenant scoping (`AUDITOR`-only cross-tenant export). Also confirmed live: exported a real signed bundle via `curl` and inspected the signature/public-key fields.

---

## ADR-014 — Milestone 11 security hardening (idempotency, request limits, CORS, monitoring)

**Status:** Accepted, implemented and tested (2026-08-20). Grouped into one ADR for the several closure-matrix items closed together in this milestone.

**Idempotency/replay protection (item 4, `SEC-06`):** an optional `Idempotency-Key` request header on `POST /audit/events`. Checked *after* acquiring the `chain_head` lock (`ADR-011`), so the check is automatically race-safe — writes are already serialized, so two concurrent requests with the same key cannot both pass the "not found" check. A duplicate key returns the original record with `200 OK` (not `201 Created`, since nothing new was created). Enforced twice: application-level lookup, and a database unique index on `(tenant_id, idempotency_key)` as a second line of defense (NULLs are distinct under standard unique-index semantics, so omitting the header is unaffected).

**Request/body limits (item 5, `SEC-06`):** `RequestSizeLimitFilter` rejects (413) any request whose declared `Content-Length` exceeds a configured limit (default 1 MiB), before the request reaches any controller. **Documented limitation:** this checks the *declared* length, not actual bytes streamed — a request sent without `Content-Length` (chunked transfer-encoding) would not be caught. Closing that gap would need a size-limiting `InputStream` wrapper instead; not implemented, given the added complexity relative to how this API is actually used (every client here sends `Content-Length`).

**CORS (item 6, `SEC-06`):** an explicit, empty allowed-origins policy (`CorsConfig`) — no origin gets cross-origin access. This is a stated decision, not silence: the assignment states no external browser-based consumer is required. Adding a real consumer later means adding its exact origin, never a wildcard.

**Operational monitoring (item 10, `SEC-09`):** Spring Boot Actuator's `/actuator/health` endpoint only (not `/metrics`, `/env`, etc. — those can leak configuration and are deliberately not exposed). Exempted from JWT auth, matching standard practice for infrastructure health checks. This is a minimal, honest starting point, not a claim of full observability — a real deployment would add metrics/tracing/alerting on top.

**Not implemented this milestone, documented instead:**
- **TLS/HTTPS deployment evidence (item 8, `SEC-09`).** Not implemented: generating and wiring a local keystore correctly, and verifying it under time pressure, carried more risk of an untested half-working config than value for a prototype whose actual deployment target doesn't exist. A real deployment needs `server.ssl.*` configured against a certificate from a real CA (or, for local reproduction, a self-signed keystore via `keytool`) — documented as the concrete next step, not attempted here.
- **Immutable DB permissions (item 9, `SEC-09`).** Investigated and found in genuine tension with this system's own design: redaction (`ADR-003`) and archival (`ADR-004`) both require the application's own DB identity to `UPDATE` `audit_record` legitimately, so a blanket "no UPDATE" grant would break real functionality this system needs. A correct solution needs column-level grants (restricting `UPDATE` to only `payload`/`status`/`redacted_fields`/`redacted_at`/`redacted_by`, never `record_hash`/`previous_hash`/`sequence_no`/`tenant_id`/`field_commitments`/`salt`) or a database with stronger row/column security than H2 offers (`ADR-001`). Not implemented given the remaining time; the actual constraint (which columns must stay immutable) is precisely documented here as the specification for whoever implements it.
- **Database fault/rollback tests (item 11, `TEST-06`) and PII/log-injection tests (item 18, `SEC-08`).** Not added this milestone — deprioritized in favor of closing more closure-matrix items with working, tested code in the remaining time, rather than partially covering more items. Recorded here as genuinely open, not silently dropped.

**Validated by:** `SecurityHardeningTest` (5 tests): idempotent replay returns the same record with 200; a request without a key is unaffected; an oversized declared-`Content-Length` request is rejected with 413; a cross-origin request gets no `Access-Control-Allow-Origin` header; `/actuator/health` is reachable without authentication.
